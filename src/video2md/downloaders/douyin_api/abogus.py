...
class ABogus:
    ...
    def generate_browser_info(cls, platform: str = "Win32") -> str:
        ...
        value_list = [inner_width, inner_height, outer_width, outer_height, 0, choice((0, 30)), 0, 0, outer_width, outer_height, outer_width, outer_height, inner_width, inner_height, 24, 24, platform]
    ...
    def get_value(self, url_params: dict | str, method="GET") -> str:
        s1 = self.generate_string_1()
        s2 = self.generate_string_2(urlencode(url_params, quote_via=quote) if isinstance(url_params, dict) else url_params, method)
        return self.generate_result(s1 + s2, "s4")
